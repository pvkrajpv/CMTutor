...
public class PitchProcessor implements AudioProcessor {
	...
	public enum PitchEstimationAlgorithm {
		 ...
		 */
		public PitchDetector getDetector(float sampleRate,int bufferSize, int windowLength,   int windowShift){
			...
			if (this == MPM ) {
				...
				detector = new Yin(sampleRate, bufferSize,windowLength,windowShift);
			}
			return detector;
		}
	...
	public PitchProcessor(PitchEstimationAlgorithm algorithm, float sampleRate,
			int bufferSize, int windowLength,   int windowShift,
			PitchDetectionHandler handler) {
		detector = algorithm.getDetector(sampleRate, bufferSize, windowLength, windowShift);
